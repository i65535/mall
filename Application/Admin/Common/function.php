...
{
    $bonus = array();
    $sql = 'SELECT type_id, type_name, type_money FROM ' . C('DB_PREFIX') . 'bonus_type ' . ' WHERE send_type = 3';
    $res = M()->query($sql);
...
{
    $rank_list = array();
    $sql = 'SELECT rank_id, rank_name, min_points FROM ' . C('DB_PREFIX') . 'user_rank ';
    if ($is_special) {
...
{
    $user_list = array();
    $sql = 'SELECT user_id, user_name FROM ' . C('DB_PREFIX') . 'users ' . $where . ' ORDER BY user_id DESC';
    $res = M()->query($sql);
...
{
    $position_list = array();
    $sql = 'SELECT position_id, position_name, ad_width, ad_height ' . 'FROM ' . C('DB_PREFIX') . 'ad_position ';
    $res = M()->query($sql);
...
{
	include_once(BASE_PATH."fckeditor/fckeditor.php");
    global $smarty;
    
    $editor = new FCKeditor($input_name);
    $editor->BasePath = BASE_PATH . 'fckeditor/';
...
function get_goods_list($filter)
{
    ...
    /* 取得数据 */
    $sql = 'SELECT goods_id, goods_name, shop_price ' . 'FROM ' . C('DB_PREFIX') . 'goods ' . ' AS g ' . $where . 'LIMIT 50';
    $row = M()->query($sql);
...
function get_article_list($filter)
{
    ...
    /* 取得数据 */
    $sql = 'SELECT a.article_id, a.title ' . 'FROM ' . C('DB_PREFIX') . 'article ' . ' AS a, ' . C('DB_PREFIX') . 'article_cat ' . ' AS c ' . $where;
    $res = M()->query($sql);
...
{
    $sql = 'SELECT cat_id, cat_name FROM ' . C('DB_PREFIX') . 'goods_type WHERE enabled = 1';
    $res = M()->query($sql);
...
function get_pay_ids()
{
    ...
    $sql = 'SELECT pay_id, is_cod FROM ' . C('DB_PREFIX') . 'payment ' . ' WHERE enabled = 1';
    $res = M()->query($sql);
...
{
    $sql = 'TRUNCATE TABLE ' . C('DB_PREFIX') . $table_name;
...
function admin_info()
{
    $sql = "SELECT * FROM " . C('DB_PREFIX') . 'admin_user ' . "
...
{
    $sql = "SELECT * FROM " . C('DB_PREFIX') . 'user_rank ' . " ORDER BY min_points";
...
function get_member_price_list($goods_id)
{
    /* 取得会员价格 */
    $price_list = array();
    $sql = "SELECT user_rank, user_price FROM " . C('DB_PREFIX') . 'member_price ' . " WHERE goods_id = '$goods_id'";
    $res = M()->query($sql);
    foreach ($res as $key => $row) {
...
function handle_goods_attr($goods_id, $id_list, $is_spec_list, $value_price_list)
{
    ...
    foreach ($id_list as $key => $id) {
        ...
        $result_id = M()->table('goods_attr')
            ...
            ->getOne();
        if (! empty($result_id)) {
            $sql = "UPDATE " . C('DB_PREFIX') . 'goods_attr ' . "
            ...
            $sql = "INSERT INTO " . C('DB_PREFIX') . 'goods_attr ' . " (goods_id, attr_id, attr_value, attr_price) " . "VALUES ('$goods_id', '$id', '$value', '$price')";
        }
...
function handle_member_price($goods_id, $rank_list, $price_list)
{
    ...
    foreach ($rank_list as $key => $rank) {
        ...
        if (M()->table('member_price')
            ...
            if ($price < 0) {
                $sql = "DELETE FROM " . C('DB_PREFIX') . 'member_price ' . " WHERE goods_id = '$goods_id' AND user_rank = '$rank' LIMIT 1";
            } else {
                $sql = "UPDATE " . C('DB_PREFIX') . 'member_price ' . " SET user_price = '$price' " . "WHERE goods_id = '$goods_id' " . "AND user_rank = '$rank' LIMIT 1";
            }
        } else {
            if ($price == - 1) {
                $sql = '';
            } else {
                $sql = "INSERT INTO " . C('DB_PREFIX') . 'member_price ' . " (goods_id, user_rank, user_price) " . "VALUES ('$goods_id', '$rank', '$price')";
            }
        }
...
{
    /* 查询现有的扩展分类 */
    $sql = "SELECT cat_id FROM " . C('DB_PREFIX') . 'goods_cat ' . " WHERE goods_id = '$goods_id'";
    $exist_list = M()->getCol($sql);
    
    /* 删除不再有的分类 */
    $delete_list = array_diff($exist_list, $cat_list);
    if ($delete_list) {
        $sql = "DELETE FROM " . C('DB_PREFIX') . 'goods_cat ' . " WHERE goods_id = '$goods_id' " . "AND cat_id " . db_create_in($delete_list);
        M()->query($sql);
    }
    ...
    foreach ($add_list as $cat_id) {
        // 插入记录
        $sql = "INSERT INTO " . C('DB_PREFIX') . 'goods_cat ' . " (goods_id, cat_id) " . "VALUES ('$goods_id', '$cat_id')";
        M()->query($sql);
    }
}
...
{
    $sql = "UPDATE " . C('DB_PREFIX') . 'link_goods ' . " SET " . " goods_id = '$goods_id' " . " WHERE goods_id = '0'" . " AND admin_id = '$_SESSION[admin_id]'";
    M()->query($sql);
    
    $sql = "UPDATE " . C('DB_PREFIX') . 'link_goods ' . " SET " . " link_goods_id = '$goods_id' " . " WHERE link_goods_id = '0'" . " AND admin_id = '$_SESSION[admin_id]'";
    M()->query($sql);
}
...
{
    $sql = "UPDATE " . C('DB_PREFIX') . 'group_goods ' . " SET " . " parent_id = '$goods_id' " . " WHERE parent_id = '0'" . " AND admin_id = '$_SESSION[admin_id]'";
    M()->query($sql);
}
...
{
    $sql = "UPDATE " . C('DB_PREFIX') . 'goods_article ' . " SET " . " goods_id = '$goods_id' " . " WHERE goods_id = '0'" . " AND admin_id = '$_SESSION[admin_id]'";
    M()->query($sql);
}
...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls)
{
    ...
    foreach ($image_descs as $key => $img_desc) {
        ...
        if ($flag) {
            ...
            $sql = "INSERT INTO " . C('DB_PREFIX') . 'goods_gallery ' . " (goods_id, img_url, img_desc, thumb_url, img_original) " . "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original')";
            M()->query($sql);
            /* 不保留商品原图的时候删除原图 */
            if ($proc_thumb && ! C('retain_original_img') && ! empty($img_original)) {
                M()->query("UPDATE " . C('DB_PREFIX') . 'goods_gallery ' . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
                @unlink('../' . $img_original);
            }
...
function update_goods($goods_id, $field, $value)
{
    if ($goods_id) {
        ...
        $sql = "UPDATE " . C('DB_PREFIX') . 'goods ' . " SET $field = '$value' , last_update = '" . gmtime() . "' " . "WHERE goods_id " . db_create_in($goods_id);
        return M()->query($sql);
    } else {
        return false;
    }
...
function delete_goods($goods_id)
{
    ...
    /* 取得有效商品id */
    $sql = "SELECT DISTINCT goods_id FROM " . C('DB_PREFIX') . 'goods ' . " WHERE goods_id " . db_create_in($goods_id) . " AND is_delete = 1";
    $arr = M()->query($sql);
    if (empty($arr)) {
        return;
    }else{
    ...
    /* 删除商品图片和轮播图片文件 */
    $sql = "SELECT goods_thumb, goods_img, original_img " . "FROM " . C('DB_PREFIX') . 'goods ' . " WHERE goods_id " . db_create_in($goods_id);
    $res = M()->query($sql);
    ...
    /* 删除商品 */
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'goods ' . " WHERE goods_id " . db_create_in($goods_id);
    M()->query($sql);
    
    /* 删除商品的货品记录 */
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'products ' . " WHERE goods_id " . db_create_in($goods_id);
    M()->query($sql);
    
    /* 删除商品相册的图片文件 */
    $sql = "SELECT img_url, thumb_url, img_original " . "FROM " . C('DB_PREFIX') . 'goods_gallery ' . " WHERE goods_id " . db_create_in($goods_id);
    $res = M()->query($sql);
    foreach ($res as $key => $row) {
    ...
    /* 删除商品相册 */
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'goods_gallery ' . " WHERE goods_id " . db_create_in($goods_id);
    M()->query($sql);
    
    /* 删除相关表记录 */
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'collect_goods ' . " WHERE goods_id " . db_create_in($goods_id);
    M()->query($sql);
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'goods_article ' . " WHERE goods_id " . db_create_in($goods_id);
    M()->query($sql);
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'goods_attr ' . " WHERE goods_id " . db_create_in($goods_id);
    M()->query($sql);
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'goods_cat ' . " WHERE goods_id " . db_create_in($goods_id);
    M()->query($sql);
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'member_price ' . " WHERE goods_id " . db_create_in($goods_id);
    M()->query($sql);
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'group_goods ' . " WHERE parent_id " . db_create_in($goods_id);
    M()->query($sql);
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'group_goods ' . " WHERE goods_id " . db_create_in($goods_id);
    M()->query($sql);
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'link_goods ' . " WHERE goods_id " . db_create_in($goods_id);
    M()->query($sql);
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'link_goods ' . " WHERE link_goods_id " . db_create_in($goods_id);
    M()->query($sql);
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'tag ' . " WHERE goods_id " . db_create_in($goods_id);
    M()->query($sql);
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'comment ' . " WHERE comment_type = 0 AND id_value " . db_create_in($goods_id);
    M()->query($sql);
    
    /* 删除相应虚拟商品记录 */
    $sql = "DELETE FROM " . C('DB_PREFIX') . 'virtual_card ' . " WHERE goods_id " . db_create_in($goods_id);
...
function generate_goods_sn($goods_id)
{
    ...
    $sn_list = M()->table('goods')->field('goods_sn')->where(" goods_sn LIKE '" . mysql_like_quote($goods_sn) . "%' AND goods_id <> '$goods_id' ")->order('LENGTH(goods_sn) DESC')->getCol();
...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price " . "FROM " . C('DB_PREFIX') . 'attribute' . " AS a " . "LEFT JOIN " . C('DB_PREFIX') . 'goods_attr' . " AS v " . "ON v.attr_id = a.attr_id AND v.goods_id = '$goods_id' " . "WHERE a.cat_id = " . intval($cat_id) . " OR a.cat_id = 0 " . "ORDER BY a.sort_order, a.attr_type, a.attr_id, v.attr_price, v.goods_attr_id";
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    $html = '<table width="100%" id="attrTable" class="table table-hover ectouch-table">';
    $spec = 0;
...
{
    $sql = "SELECT lg.link_goods_id AS goods_id, g.goods_name, lg.is_double " . "FROM " . C('DB_PREFIX') . 'link_goods ' . " AS lg, " . C('DB_PREFIX') . 'goods ' . " AS g " . "WHERE lg.goods_id = '$goods_id' " . "AND lg.link_goods_id = g.goods_id ";
    if ($goods_id == 0) {
...
{
    $sql = "SELECT gg.goods_id, CONCAT(g.goods_name, ' -- [', gg.goods_price, ']') AS goods_name " . "FROM " . C('DB_PREFIX') . 'group_goods ' . " AS gg, " . C('DB_PREFIX') . 'goods ' . " AS g " . "WHERE gg.parent_id = '$goods_id' " . "AND gg.goods_id = g.goods_id ";
    if ($goods_id == 0) {
...
{
    $sql = "SELECT g.article_id, a.title " . "FROM " . C('DB_PREFIX') . 'goods_article ' . " AS g, " . C('DB_PREFIX') . 'article ' . " AS a " . "WHERE g.goods_id = '$goods_id' " . "AND g.article_id = a.article_id ";
    if ($goods_id == 0) {
 ...
 * @return array
 */
function goods_list($is_delete, $offset, $real_goods = 1, $conditions = '')
{
    /* 过滤条件 */
    ...
    if ($result === false) {
        ...
        $filter['record_count'] = M()->table(C('DB_PREFIX') . 'goods as g')
            ->field('COUNT(*)')
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " . 
        		...
        		" FROM " . C('DB_PREFIX') . "goods WHERE is_delete='$is_delete' $where" . 
...
function product_goods_attr_list($goods_id)
{
    ...
    $sql = "SELECT goods_attr_id, attr_value FROM " . C('DB_PREFIX') . 'goods_attr ' . " WHERE goods_id = '$goods_id'";
    $results = M()->query($sql);
...
function get_goods_specifications_list($goods_id)
{
    ...
    $sql = "SELECT g.goods_attr_id, g.attr_value, g.attr_id, a.attr_name
            FROM " . C('DB_PREFIX') . 'goods_attr ' . " AS g
                LEFT JOIN " . C('DB_PREFIX') . 'attribute ' . " AS a
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false) {
        ...
        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_number
                FROM " . C('DB_PREFIX') . 'products ' . " AS g
                WHERE goods_id = $goods_id $where
                ORDER BY $filter[sort_by] $filter[sort_order]";
...
function get_product_info($product_id, $filed = '')
{
    ...
    $sql = "SELECT $filed FROM  " . C('DB_PREFIX') . 'products ' . " WHERE product_id = '$product_id'";
...
function check_goods_specifications_exist($goods_id)
{
	...
	$sql = "SELECT COUNT(a.attr_id) as count
            FROM " . C('DB_PREFIX') . "attribute AS a, " . C('DB_PREFIX') . "goods AS g
...
function check_product_sn_exist($product_sn, $product_id = 0)
{
    ...
    $sql = "SELECT goods_id FROM " . C('DB_PREFIX') . 'goods ' . "WHERE goods_sn='$product_sn'";
    if (M()->table('goods')
        ->field('goods_id')
...
{
	$sql = 'SELECT brand_id, brand_name FROM ' . C('DB_PREFIX') . 'brand ORDER BY sort_order';
	$res = M()->query($sql);
...
/**
 ...
 function pay_fee($payment_id, $order_amount, $cod_fee = null) {
 	$pay_fee = 0;
 	$payment = model('OrderBase')->payment_info($payment_id);
